...
export function useStudentEquipment(props: { isVisible?: boolean }) {
  ...
  const { loadNow: loadEquipments, flushPendingNow: flushPendingEquipments } =
    useDebouncedFormPersistence({
      ...
      saveFn: async () => {
        const saved = await saveEquipmentInventory(equipmentFormData.value);
        if (!saved) throw new Error('Failed to save equipment inventory.');
