...
export function useGearCalculation() {
  ...
  const calculateExpNeeds = () => {
    ...
    const ownedXp = getEquipXpItems((id) => resources[id]?.QuantityOwned ?? 0, resources).reduce(
  ...
  if (!_totalEquipmentsNeeded) {
    _totalEquipmentsNeeded = computed(() => {
      ...
      if (xpBallMaterial) {
        materialMap.set(1, {
          // Using Novice exp ball ID as the XP material ID
  ...
  const getEquipmentUsageByStudents = (
    materialId: number,
    ...
    if (isExpBall(materialId)) {
      ...
      const sortedStudents = Array.from(materialNeeds.entries()).sort(
