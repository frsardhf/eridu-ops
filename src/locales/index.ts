...
const translations = {
  en: {
    ...
    dataLoadError:
      'Could not load student data from SchaleDB. Check your connection and try again.',
    retry: 'Retry',
    ...
    importInstructions:
      'Import your previously exported data to restore your progress and settings.',
  ...
  jp: {
    ...
    syncGiftsModeAwareDesc:
      '他のキャラクターの専用ギア強化に必要なギフトを確保してから設定します。',
    // Convert material selection dialog
    ...
    lowConfidenceWarning:
      '信頼度の低いアイテムは誤認識の可能性があります。適用前にご確認ください。',
    noItemsDetected:
    ...
    scanGuide: {
      ...
      screenshots: {
        ...
        itemsTab:
          '<strong>アイテム</strong>タブで検出されるもの：経験値素材、秘伝記、戦術教育BD、技術ノート、贈り物（1ページ4行）。',
        equipmentTab:
    ...
    scanModal: {
      ...
      prepHintItems:
        '<strong>スクリーンショットを撮る前に：</strong>ゲーム内フィルターで経験値素材、秘伝記、戦術教育BD、技術ノート、贈り物のみを表示してください。それ以外のカテゴリは検出されません。',
      prepHintEquipment:
    ...
    bulkModify: {
      ...
      formNote:
        '空欄の項目は既存の値を保持します。データのない生徒にはデフォルト値が使用されます。',
      fields: {
        bond: '絆',
