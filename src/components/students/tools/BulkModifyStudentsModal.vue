...
function buildPatchFromFields(): BulkFormPatch {
  ...
  return {
    bondLevel: parseOptionalInt(fv.bond, MIN_BOND_LEVEL, MAX_BOND_LEVEL),
    characterLevel: parseOptionalInt(fv.characterLevel, 1, MAX_LEVEL),
    characterLevelTarget: targets ? parseOptionalInt(fv.characterLevelTarget, 1, MAX_LEVEL) : null,
    skillEx: parseOptionalInt(fv.skillEx, 1, MAX_EX_SKILL_LEVEL),
    skillExTarget: targets ? parseOptionalInt(fv.skillExTarget, 1, MAX_EX_SKILL_LEVEL) : null,
...
const gradeStarDisplay = computed(() => {
  ...
  if (!raw) return null;
  const grade = Math.max(1, Math.min(MAX_GRADE, Math.floor(Number(raw))));
  if (!Number.isFinite(grade)) return null;
  return {
    starCount: grade <= WEAPON_STAR_THRESHOLD ? grade : grade - WEAPON_STAR_THRESHOLD,
    isGold: grade <= WEAPON_STAR_THRESHOLD,
  };
});
